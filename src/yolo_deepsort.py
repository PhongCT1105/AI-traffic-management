...
class Video_Tracker(object):
    def __init__(self, args, cfg):
        ...
        if args.save_path:
            ...
            self.save_video_path = os.path.join(args.save_path, "results.avi")
    ...
    def run(self):
        idx_frame = 0
        cal_speed = vehicle_speed(vid_fps=30, distance=10)
        while self.vdo.grab():
            idx_frame += 1
            ...
            # deepsort
            line_1 = [(700, 618), (1150, 618)]
            line_2 = [(592, 790), (1250, 790)]

            line_3 = [(1200, 618), (1800, 618)]
            line_4 = [(1300, 790), (1920, 790)]
            # draw line
            cv2.line(im0, line_1[0], line_1[1], (0, 255, 0), 2)
            cv2.line(im0, line_2[0], line_2[1], (0, 0, 255), 2)

            cv2.line(im0, line_4[0], line_4[1], (0, 255, 0), 2)
            cv2.line(im0, line_3[0], line_3[1], (0, 0, 255), 2)

            outputs = self.deepsort.update(bbox_xywh.cpu(), cls_conf.cpu(), im0)
            # print(outputs)
            if len(outputs) > 0:
                bbox_xyxy = outputs[:, :4]
                ...
                for i, bbox in enumerate(bbox_xyxy):
                    ...
                    if (
                        y_center > line_1[0][1]
                        and y_center < line_2[0][1]
                        and x_center > line_1[0][0]
                        and x_center < line_1[1][0]
                        and id not in cal_speed.vehilce_id_list
                    ) or (
                        y_center < line_4[0][1]
                        and y_center > line_3[0][1]
                        and x_center > line_4[0][0]
                        and x_center < line_4[1][0]
                        and id not in cal_speed.vehilce_id_list
                    ):
                        cal_speed.new_vehicle(id, idx_frame)
                    elif (
                        y_center > line_1[0][1]
                        and y_center > line_2[1][1]
                        and x_center > line_2[0][0]
                        and x_center < line_2[1][0]
                        and id in cal_speed.vehilce_id_list
                    ) or (
                        y_center < line_4[0][1]
                        and y_center < line_3[1][1]
                        and x_center > line_4[0][0]
                        and x_center < line_4[1][0]
                        and id in cal_speed.vehilce_id_list
                    ):
                        ...
                        if cal_speed.speed_list[index] == 0:
                            ...
                            im0 = cv2.putText(
                                im0,
                                str(cal_speed.speed_list[index]) + "km/h",
                                (int(x_min), int(y_min)),
                                cv2.FONT_HERSHEY_SIMPLEX,
                                1,
                                (0, 0, 255),
...
def parse_args():
    ...
    parser.add_argument("--video_path", type=str, default="data/video/test.mp4")
    parser.add_argument("--detector", type=str, default="yolov5")
    parser.add_argument("--cfg_path", type=str, default="../cfgs/yolo_deepsort.yaml")
    parser.add_argument(
